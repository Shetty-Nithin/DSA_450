...
Example 1:
    ...
    would always have same or more cost. For example, if we connect
    4 and 6 first (we get three strings of 3, 2 and 10), then connect 
    10 and 3 (we get two strings of 13 and 2). Finally we connect 13 and 2.
...
Example 2:
    ...
    Explanation:
    First, connect ropes 4 and 2, which makes the array {6,7,6,9}. Next, 
    add ropes 6 and 6, which results in {12,7,9}. Then, add 7 and 9, 
    which makes the array {12,16}. And finally add these two which gives 
...
class Solution
{
    ...
    long long minCost(long long arr[], long long n) {
        ...
        while(min_heap.size() > 1){
            long long first_min = min_heap.top();
            min_heap.pop();
            
            long long second_min = min_heap.top();
            min_heap.pop();
